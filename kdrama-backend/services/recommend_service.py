...
def get_recommendations_for_user(user_id, engine, top_n=10):
    ...
    try:
        with engine.connect() as connection:
            ...
            # if not profile:
            #     # Sắp xếp theo: Rating cao nhất -> Hạng Popularity nhỏ nhất (Hot nhất) -> Lượng Watchers nhiều nhất
            #     # Lưu ý: Vì cột watchers trong DB của bạn đang là kiểu TEXT (ví dụ: '81,971'), 
            #     # chúng ta dùng REPLACE và CAST về số nguyên để sắp xếp chính xác trong SQL.
            #     fallback_query = text("""
            #         SELECT drama_id, title, genres, main_lead1, main_lead2, rating, popularity, watchers, start_year, episodes, content, directors
            #         FROM public.dramas 
            #         ORDER BY 
            #             rating DESC, 
            #             popularity ASC, 
            #             CAST(REPLACE(watchers, ',', '') AS INT) DESC
            #         LIMIT :limit
            #     """)
            ...
            for drama in all_dramas:
                ...
                #     # "match_score": score
                #     "match_score": round(score, 2) # Làm tròn điểm hợp gu cho đẹp dữ liệu
                # })
...
def update_user_profile(user_id, drama_id, action_type, rating_val, engine):
    ...
    try:
        with engine.begin() as connection:  # Dùng engine.begin() để tự động commit/rollback (Transaction)
            ...
            #         genre_w[g] = max(0.0, genre_w.get(g, 0.0) + weight_to_add) # Không để điểm xuống âm
            # if drama['actors']:
            #     for a in drama['actors']:
            #         actor_w[a] = max(0.0, actor_w.get(a, 0.0) + weight_to_add)
            # if drama['tags']:
            #     for t in drama['tags']:
            #         tag_w[t] = max(0.0, tag_w.get(t, 0.0) + weight_to_add)
