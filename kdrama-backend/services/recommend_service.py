...
#             # Khởi tạo biến chứa kết quả thô từ DB
#             raw_result = []
...
#                         rating DESC, 
#                         popularity ASC, 
#                         -- Ép kiểu nhanh sang số nguyên lúc ORDER BY để Postgres sắp xếp số lượng chính xác
#                         CAST(REGEXP_REPLACE(watchers, '[^0-9]', '', 'g') AS INT) DESC
#                     LIMIT :limit
#                 """)
...
#                         start_year, episodes, watchers, content, directors,
#                         -- TÍNH ĐIỂM THỂ LOẠI
#                         (SELECT COALESCE(SUM(val), 0.0)
#                          FROM unnest(:genre_keys::text[], :genre_vals::float[]) AS w(key, val)
#                          WHERE w.key = ANY(string_to_array(genres, ', '))) AS genre_score,
                        
#                         -- TÍNH ĐIỂM DIỄN VIÊN
#                         COALESCE((SELECT val FROM unnest(:actor_keys::text[], :actor_vals::float[]) AS w(key, val) WHERE w.key = TRIM(main_lead1)), 0.0) +
#                         COALESCE((SELECT val FROM unnest(:actor_keys::text[], :actor_vals::float[]) AS w(key, val) WHERE w.key = TRIM(main_lead2)), 0.0) AS actor_score,
                        
#                         -- TÍNH ĐIỂM TAGS
#                         (SELECT COALESCE(SUM(val), 0.0)
#                          FROM unnest(:tag_keys::text[], :tag_vals::float[]) AS w(key, val)
...
#                     start_year, episodes, watchers, content, directors,
#                     ROUND((genre_score + actor_score + tag_score)::numeric, 2) AS match_score
#                 FROM scored_dramas
...
#             result = connection.execute(recommend_query, params).mappings().all()
...
#                 # drama_dict["watchers"] = str(clean_watchers_int)
#                 # Định dạng f"{clean_watchers_int:,}" tự động thêm dấu phẩy chuẩn format Frontend cần
...
#     try:
#         with engine.begin() as connection:  # Tự động commit/rollback
...
#             # Khởi tạo hoặc parse an toàn bằng hàm helper bên trên
#             if profile:
#                 genre_w = parse_weights(profile['genre_weights'])
#                 actor_w = parse_weights(profile['actor_weights'])
#                 tag_w = parse_weights(profile['tag_weights'])
...
#             # 3. Tính toán cộng dồn điểm mới (Có giới hạn max(0.0, ...) để tránh điểm âm vô hạn)
#             if drama['genres']:
